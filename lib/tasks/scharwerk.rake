namespace :sharwerk do
  ...
  task :good_task, [:argument] => :environment do |_t, args|
    puts args[:argument]
  end

  desc 'Create all tasks and pages from texts'
  task :create_tasks, [:scans_folder_path,
                       :texts_folder_path,
                       :stage,
                       :part,
                       :pages_per_task] => :environment do |_t, args|
    part_pages = Page.create_pages(args[:scans_folder_path],
                                   args[:texts_folder_path])
...
# bundle exec rake sharwerk:create_tasks['public/scans/franko.web',
# 'public/das-kapital/text/franko','test','franko',5]
